...
def violation_loss(
    ...
    **kwargs,
    ...
    if breakdown:
        return loss, dict(
            bonds_c_n_loss_mean=violations["between_residues"]["bonds_c_n_loss_mean"],
            angles_ca_c_n_loss_mean=violations["between_residues"][
                "angles_ca_c_n_loss_mean"
            ],
            angles_c_n_ca_loss_mean=violations["between_residues"][
...
class AlphaFoldLoss(nn.Module):
    ...
    def forward(self, out, batch, _return_breakdown=False, violation_breakdown=False):
        ...
        if violation_breakdown:
            losses.update(violation_loss(out["violation"], breakdown=True,**batch)[1])
