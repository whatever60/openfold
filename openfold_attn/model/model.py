            ...
            with torch.set_grad_enabled(is_grad_enabled and is_final_iter):
                ...
                if self.config.template.enabled:
                    ...
                    blocks = [
                        ...
                        for b in self.template_pair_stack.blocks
                    ]
                    ...
                    if not self.training:
                        for i, b in enumerate(blocks):
                            ...
                            attns[f"{cycle_no}-templ-tri_s-{i}"] = attn_tri_s
                            attns[f"{cycle_no}-templ-tri_e-{i}"] = attn_tri_e
                    else:
                        (t_pair,) = checkpoint_blocks(
                            blocks=blocks,
                            args=(t_pair,),
                            blocks_per_ckpt=self.template_pair_stack.blocks_per_ckpt,
                        )

                    t = self.template_pair_stack.layer_norm(t_pair)
                    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                    ...
                    # t: [batch_size, num_templates, num_residues, num_residues, templ_pair_emb_dim]
                    # z: [batch_size, num_residues, num_residues, pair_emb_dim]
                    # mask: [batch_size, num_templates]
                    ...
                    attns[f"{cycle_no}-templ-pointwise"] = attn

                    t_mask = torch.sum(template_feats["template_mask"], dim=-1) > 0
                    # Append singletons
                    t_mask = t_mask.reshape(
                        *t_mask.shape, *([1] * (len(t.shape) - len(t_mask.shape)))
                    )
                    ...
                    if self.config.template.embed_angles:
                        template_angle_feat = build_template_angle_feat(template_feats)
                    ...
                    if self.globals.offload_inference:
                        ...
                        for i, b in enumerate(blocks):
                            ...
                            if self.extra_msa_stack.ckpt and torch.is_grad_enabled():
                                ...
                                attns[f"{cycle_no}-extra_msa-row-{i}"] = attn_row
                                attns[f"{cycle_no}-extra_msa-col-{i}"] = attn_col
                                attns[f"{cycle_no}-extra_msa-tri_s-{i}"] = attn_tri_s
                                attns[f"{cycle_no}-extra_msa-tri_e-{i}"] = attn_tri_e
                        # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                ...
                if self.globals.offload_inference:
                    ...
                    if not torch.is_grad_enabled():
                        for b in blocks:
                            ...
                            attns[f"{cycle_no}-evoformer-row-{i}"] = attn_row
                            attns[f"{cycle_no}-evoformer-col-{i}"] = attn_col
                            attns[f"{cycle_no}-evoformer-tri_s-{i}"] = attn_tri_s
                            attns[f"{cycle_no}-evoformer-tri_e-{i}"] = attn_tri_e
                    else:
                ...
                for i in range(sm.no_blocks):
                    ...
                    # [*, N_res, H, P_q, 3]
                    q_pts = q_pts.view(
                        q_pts.shape[:-2] + (ipa.no_heads, ipa.no_qk_points, 3)
                    )
                    ...
                    # [*, N_res, H, (P_q + P_v), 3]
                    kv_pts = kv_pts.view(kv_pts.shape[:-2] + (ipa.no_heads, -1, 3))

                    # [*, N_res, H, P_q/P_v, 3]
                    k_pts, v_pts = torch.split(
                        kv_pts, [ipa.no_qk_points, ipa.no_v_points], dim=-2
                    )
                    ...
                    a *= math.sqrt(1.0 / (3 * ipa.c_hidden))
