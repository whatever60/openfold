...
class EvoformerBlockCore(nn.Module):
    ...
    def forward(
        ...
        # the original.
        attn_tri_s, attn_tri_e = None, None
        ...
        #     inplace=inplace_safe,
        # )
        z_update, attn_tri_s = self.tri_att_start(
            z,
            mask=pair_mask,
            chunk_size=_attn_chunk_size,
        ...
        z = add(z, self.ps_dropout_row_layer(z_update), inplace=inplace_safe)
        ...
        #     inplace=inplace_safe,
        # )
        z_update, attn_tri_e = self.tri_att_end(
            z,
            mask=pair_mask.transpose(-1, -2),
            chunk_size=_attn_chunk_size,
            use_memory_efficient_kernel=True,
        ...
        return m, z, attn_tri_s, attn_tri_e
...
class EvoformerBlock(nn.Module):
    ...
    def forward(
        ...
        attn_row, attn_col, attn_tri_s, attn_tri_e = None, None, None, None
        if _attn_chunk_size is None:
            _attn_chunk_size = chunk_size
        ...
        #     inplace=inplace_safe,
        # )
        m_update, attn_row = self.msa_att_row(
            m,
            z=z,
            mask=msa_mask,
            chunk_size=_attn_chunk_size,
        ...
        m = add(m, self.msa_dropout_layer(m_update), inplace=inplace_safe)
        ...
        #     inplace=inplace_safe,
        # )
        m_update, attn_col = self.msa_att_col(
            m,
            mask=msa_mask,
            chunk_size=chunk_size,
            use_lma=use_lma,
        ...
        m = add(m, m_update, inplace=inplace_safe)
        ...
        m, z, attn_tri_s, attn_tri_e = self.core(
            input_tensors,
            msa_mask=msa_mask,
            pair_mask=pair_mask,
            chunk_size=chunk_size,
        ...
        return m, z, attn_row, attn_col, attn_tri_s, attn_tri_e
...
class ExtraMSABlock(nn.Module):
    ...
    def forward(
        ...
        attn_col, attn_col, attn_tri_s, attn_tri_e = None, None, None, None
        if _attn_chunk_size is None:
            _attn_chunk_size = chunk_size
        ...
        def fn(input_tensors):
            ...
            m = add(input_tensors[0], m_update, inplace=inplace_safe)
            ...
            return m, z, attn_col, attn_tri_s, attn_tri_e

        if torch.is_grad_enabled() and self.ckpt:
            checkpoint_fn = get_checkpoint_fn()
            m, z = checkpoint_fn(fn, input_tensors)
        else:
            m, z, attn_col, attn_tri_s, attn_tri_e = fn(input_tensors)
